...
public class Version{
	...
	public Command getLaunchCommand(Arguments arguments, Path javaPath){
		...
		{
			for (int i = 0; i < command.size(); i++) if (command.get(i).startsWith("-Xms"))
			{
				command.remove(i);
				break;
			}
		...
		{
			for (int i = 0; i < command.size(); i++) if (command.get(i).startsWith("-Xmx"))
			{
				command.remove(i);
				break;
			}
		...
		List<Variable> variables = new ArrayList<>(List.of(
				new Variable("${launcher_name}", "mod-profiles-relauncher"),

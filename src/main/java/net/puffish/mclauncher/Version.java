...
public class Version
{
    ...
    public Command getLaunchCommand(Arguments arguments, Path javaPath)
    {
        ...
        // Add mod file to classpath
        ModProfilesMod.LOGGER.info("Relauncher Source: " + ModProfilesMod.MOD_FILE_PATH);
        librariesString += os.getClasspathSeparator() + ModProfilesMod.MOD_FILE_PATH;
        ...
        // Copy in old launch arguments
        arguments.forEachVariable(arg ->
        {
            String[] tokens = arg.split("=");
            if (command.stream().noneMatch(str -> str.contains(tokens[0]))) command.add(arg);
        });
        
        // Copy in old RAM arguments
        arguments.getMinRamArgument().ifPresent(minRam ->
        {
            for (int i = 0; i < command.size(); i++)
                if (command.get(i).startsWith("-Xms"))
                {
                    command.remove(i);
                    break;
                }
        ...
        arguments.getMaxRamArgument().ifPresent(maxRam ->
        {
            for (int i = 0; i < command.size(); i++)
        ...
        List<Variable> variables = new ArrayList<>(List.of(
                new Variable("${launcher_name}", Relauncher.LAUNCHER_NAME),
                new Variable("${launcher_version}", VersionChecker.getModVersion()),
                
                new Variable("${auth_player_name}", arguments.getSession().getUsername()),
                new Variable("${auth_uuid}", arguments.getSession().getUuid()),
                new Variable("${auth_access_token}", arguments.getSession().getAccessToken()),
                new Variable("${user_type}", arguments.getSession().getAccountType().getName()),
                new Variable("${clientid}", arguments.getSession().getClientId().orElse("0000")),
                new Variable("${auth_xuid}", arguments.getSession().getXuid().orElse("0000")),
                new Variable("${auth_session}", arguments.getSession().getSessionId()),
                
                new Variable("${version_name}", versionName),
                new Variable("${game_directory}", arguments.getRunDirectory().toAbsolutePath().toString()),
                new Variable("${assets_root}", gd.assets().toAbsolutePath().toString()),
                new Variable("${user_properties}", "{}"),
                new Variable("${version_type}", arguments.getOriginalArg("--versionType").orElse("Fabric")),
                new Variable("${game_assets}", "resources"),
                //new Variable("${classpath}", librariesString, mainClass),
                new Variable("${classpath}", librariesString, Relauncher.ENTRY_POINT),
